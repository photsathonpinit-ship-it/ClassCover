      ...
      <form action={updateTeacher} className="bg-white rounded-lg shadow-sm p-4 sm:p-6 space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div>
            ...
            <input name="title" defaultValue={teacher.title} className="w-full border rounded-md px-3 h-11 text-sm" />
          </div>
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1">ชื่อ</label>
            <input name="firstName" required defaultValue={teacher.firstName} className="w-full border rounded-md px-3 h-11 text-sm" />
          </div>
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1">นามสกุล</label>
            <input name="lastName" required defaultValue={teacher.lastName} className="w-full border rounded-md px-3 h-11 text-sm" />
          </div>
        </div>

        <div>
          <label className="block text-sm font-medium text-slate-700 mb-1">วิชาที่สอนได้</label>
          <input name="subjectGroups" defaultValue={groups.join(", ")} className="w-full border rounded-md px-3 h-11 text-sm" />
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1">เบอร์โทร</label>
            <input name="phone" defaultValue={teacher.phone ?? ""} className="w-full border rounded-md px-3 h-11 text-sm" />
          </div>
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1">อีเมล</label>
            <input name="email" defaultValue={teacher.email ?? ""} className="w-full border rounded-md px-3 h-11 text-sm" />
          </div>
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1">คาบสูงสุด/วัน</label>
            <input name="maxPeriodsPerDay" type="number" defaultValue={teacher.maxPeriodsPerDay} className="w-full border rounded-md px-3 h-11 text-sm" />
          </div>
        </div>

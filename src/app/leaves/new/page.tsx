...
export default async function NewLeavePage({ searchParams }: PageProps<"/leaves/new">) {
  ...
  return (
    <div className="p-8 max-w-2xl">
      ...
      <form action={createLeave} className="bg-white rounded-lg shadow-sm p-6 space-y-4">
        <div>
          <label className="block text-sm font-medium text-slate-700 mb-1">ครู</label>
          <select name="teacherId" required className="w-full border rounded-md px-3 py-2 text-sm bg-white">
        ...
        <div>
          ...
          <select name="leaveType" className="w-full border rounded-md px-3 py-2 text-sm bg-white">
            {LEAVE_TYPES.map((lt) => (
        ...
        <div className="flex gap-3 pt-2">
          ...
          <Link href="/leaves" className="bg-slate-100 hover:bg-slate-200 text-slate-700 px-5 py-2 rounded-md text-sm font-medium">
            ยกเลิก
          </Link>
        </div>
      </form>
